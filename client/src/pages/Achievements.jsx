...
import {
  Trophy, Star, Target, Flame, Zap, Shield, Database,
  Lock, Award, Clock, Crown, Gift, ArrowRight, Loader
} from 'lucide-react';
...
// ── BADGE UNLOCK LOGIC — driven by real stats ────────────
// stats shape: { totalSolved, currentStreak, longestStreak,
//               totalPoints, dsaSolved, pureSolves, speedBadges }
const getBadgeState = (badge, stats) => {
  let current = 0;

  if (badge.type === 'streak')  current = stats.longestStreak  || 0;
  if (badge.type === 'solved')  current = stats.totalSolved    || 0;
  if (badge.type === 'dsa')     current = stats.dsaSolved      || 0;
  if (badge.type === 'points')  current = stats.totalPoints    || 0;
  if (badge.type === 'pure')    current = stats.pureSolves     || 0;
  if (badge.type === 'speed') {
    const sb = stats.speedBadges || {};
    if (badge.id === 'speed_easy')   current = sb.easy   ? 1 : 0;
    if (badge.id === 'speed_medium') current = sb.medium ? 1 : 0;
    if (badge.id === 'speed_hard')   current = sb.hard   ? 1 : 0;
  }
    ...
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="text-right">
          ...
          {next && (
            <p className="text-zinc-600 text-[9px] font-bold">
              {(next - points).toLocaleString()} to {LEVEL_NAMES[level + 1]}
            </p>
          )}
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────
const BadgeCard = ({ badge, stats, Icon }) => {
  const { unlocked, pct, remaining } = getBadgeState(badge, stats);
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between">
        ...
        <span
          className="text-[8px] font-black px-1.5 py-0.5 rounded border uppercase tracking-wider"
          style={{
      ...
      <div className="mt-auto">
        <div className="h-1 bg-zinc-800 rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
            animate={{ width: `${unlocked ? 100 : Math.max(pct, 5)}%` }}
...
const Achievements = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${NODE_URL}/api/dashboard/${user.uid}`);

        if (!res.ok) throw new Error(`Server error: ${res.status}`);

        const data = await res.json();

        // data.stats contains all the numbers we need.
        // The backend (after Step 1) will include:
        //   totalSolved, currentStreak, longestStreak, globalRank,
        //   totalPoints, hintsUsed, pureSolves, dsaSolved,
        //   totalUsers, speedBadges
        setStats(data.stats);
      } catch (err) {
  ...
  // ── DERIVED VALUES ───────────────────────────────────────
  // Only computed once stats are loaded
  const level      = stats ? getLevel(stats.totalPoints) : 0;
  const topPct     = stats ? Math.round(((stats.globalRank || 1) / (stats.totalUsers || 1)) * 100) : 0;
  const allUnlocked = stats
    ? BADGE_GROUPS.flatMap(g => g.badges).filter(b => getBadgeState(b, stats).unlocked).length
    : 0;
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center gap-4">
        ...
        >
          <Loader size={28} className="text-cyan-500" />
        </motion.div>
        <p className="text-zinc-600 text-xs font-bold tracking-widest uppercase">Loading achievements…</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-zinc-400 text-sm font-bold">Couldn't load achievements</p>
        <p className="text-zinc-600 text-xs">{error}</p>
        <button
          onClick={() => window.location.reload()}
          className="mt-2 px-4 py-2 bg-zinc-900 border border-zinc-700 rounded-lg text-xs text-zinc-400 hover:text-white transition-colors"
        >
          Retry
        </button>
      </div>
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <p className="text-zinc-500">Please log in to see your achievements.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-12">
        ...
        {/* ── TOP ROW: XP + BADGE PROGRESS ────────────────── */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-8">
          <div className="lg:col-span-2">
            <XPBar points={stats?.totalPoints ?? 0} />
          </div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5 flex flex-col justify-between">
            ...
            <div>
              <p className="text-4xl font-black text-white">
                {allUnlocked}
                <span className="text-zinc-600 text-xl">/{totalBadges}</span>
              </p>
          ...
          {[
            {
              ...
              icon:  Target,
              color: '#00E5FF',
              sub:   `${Math.round(((stats?.totalSolved ?? 0) / 173) * 100)}% of all problems`,
            },
            {
              label: 'Total Points',
              ...
              icon:  Star,
              color: '#FBBF24',
              sub:   `Level ${level} · ${LEVEL_NAMES[level]}`,
            },
            {
              label: 'Pure Solves',
              value: stats?.pureSolves ?? 0,
              icon:  Shield,
              color: '#8B5CF6',
            ...
            },
            {
              ...
              icon:  Flame,
              color: '#F97316',
              sub:   `Current: ${stats?.currentStreak ?? 0} days`,
            },
          ].map((s, i) => {
            const Icon = s.icon;
            return (
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {visibleGroups.map(group => {
              ...
              const unlockedInGroup = group.badges.filter(b => getBadgeState(b, stats).unlocked).length;
                ...
                <div key={group.group}>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[9px] text-zinc-700 font-bold">
                      {unlockedInGroup}/{group.badges.length} unlocked
                    </span>
                  </div>
        ...
        <motion.div
          ...
          style={{
            background: 'linear-gradient(135deg, rgba(9,9,11,1) 0%, rgba(0,229,255,0.04) 50%, rgba(139,92,246,0.06) 100%)',
          }}
        >
